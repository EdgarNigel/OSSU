...
def updateHand(hand, word):
    ...
    for i in range(len(word)):
        if word[i] in updatedHand.keys():
            updatedHand[word[i]] = updatedHand.get(word[i], 0) - 1
...
def isValidWord(word, hand, wordList):
    ...
    if word in wordList:    
        for i in range(len(word)):
            if word[i] not in checkHand.keys():
                return False
            else:
                if checkHand[word[i]] == 0:
                    return False
                else:
                    checkHand[word[i]] = checkHand.get(word[i], 0) - 1
    ...
    while calculateHandlen(hand) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                print(str(userInput) + " earned " + str(getWordScore(userInput, n)) + " points. Total: " + str(score) + " points.")
                print()
                # Update the hand 
